...
test("M1 imports a unitless RGB-D pair and commits a Scene", async ({ page }) => {
  ...
  const pfm = Buffer.from("Pf\n2 2\n1.0\n\x00\x00\x00?\x00\x00\x00?\x00\x00\x80?\x00\x00\x80?", "binary");
  ...
  await expect(page.getByText("DEPTH_SEMANTICS_REQUIRED")).toBeVisible();
  ...
  await expect(page.getByRole("heading", { name: "Untitled Scene", exact: true })).toBeVisible();
