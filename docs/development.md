...
diagnostics and API responses. The browser owns interaction state and previews. M1 does not import
device, model, CUDA, robot or external-repository code.
...
npm run typecheck
npm run build
npm run test:e2e -- tests/e2e/m1-import.spec.ts
```
...
layout. The test web server uses a deterministic session token only inside the test process; normal
...
M2 should consume `SceneManifestV1`, `ProcessingSpecV1` and the staged source contract without changing
their meaning. M3 should consume an applied derivation and introduce the versioned CameraPath/RenderSpec
contracts. Do not add point-cloud or video behavior to the M1 import endpoint as an implicit side effect.
